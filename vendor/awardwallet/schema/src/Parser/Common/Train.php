...
class Train extends Itinerary {
	...
	public function addSegment() {
		$segment = new TrainSegment(sprintf('%s-%d', $this->_name, $this->_seg_cnt), $this->logger, $this->_options);
		$this->_seg_cnt++;
		$this->segments[] = $segment;
		$this->logDebug($this->getId() . ': created new train segment ' . $segment->getId());
		return $segment;
	}

	/**
	 * @param TrainSegment $segment
	 * @return Train
	 */
	public function removeSegment(TrainSegment $segment) {
		$idx = null;
    ...
    public function sortSegments()
    {
        ...
        usort($this->segments, function(TrainSegment $a, TrainSegment $b){
