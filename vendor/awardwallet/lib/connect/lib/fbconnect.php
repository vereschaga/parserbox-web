...
/*   FUNCTION OVERVIEW
 *   render_fbconnect_init_js()					RENDERS JAVASCRIPT TO INITIALIZE CONNECT
 *   render_fbconnect_button($size='medium')	RENDERS FB CONNECT BUTTON
 *   register_feed_form_js($run)				DISPLAYS FEED FORM WHEN PAGE IS LOADED
 *   facebook_client()							RETURNS FACEBOOK CLIENT CLASS
 *   facebook_registerUsers($accounts) 			REGISTER NEW ACCOUNTS WITH FACEBOOK TO FACILITATE FRIEND LINKING
 *   facebook_unregisterUsers($email_hashes)	UNREGISTERS ARRAY OF EMAIL HASHES ON FACEBOOK FROM YOUR SITE
 *   facebook_get_fields($fb_uid, $fields)		RETURNS FIELDS FROM USER'S FB ACCOUNT
 *   email_get_public_hash($email)				RETURNS PUBLIC EMAIL HASH
 *   ensure_loaded_on_correct_url()				ENSURES URL MATCHES FB APPLICATION SETTINGS
 *   facebook_client()							RETURNS FACEBOOK CLIENT CLASS
 */
...
function render_fbconnect_init_js() {
	$html = sprintf(
		'<script src="%s/js/api_lib/v0.4/FeatureLoader.js.php" type="text/javascript"></script>
		 ...
		 <script src="' . CONNECT_JAVASCRIPT_PATH . 'fbconnect.js" type="text/javascript"></script>',
		get_static_root(),
		get_api_key());
...
function register_feed_form_js($run) {
	onloadRegister(sprintf("facebook_publish_feed_story(%d, %s); ", get_feed_bundle_id(), json_encode($run->getTemplateData())));
}
