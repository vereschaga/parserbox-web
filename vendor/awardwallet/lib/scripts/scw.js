...
//******************************************************************************
    ...
    var scwBaseYear        = scwDateNow.getFullYear()-120;

    // How many years do want to be valid and to show in the drop-down list?

    var scwDropDownYears   = 160;
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            '/* Calendar background */'                                 +
            'table.scw                  {background-color:	#e2eef6;'	+
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwNow                  {background-color:  #e2eef6;'   +
                                        'color:             #6666CC;}'  +
            '/* Clear Button */'                                        +
...
// ****************************************************************************
    ...
    function scwShow(scwEle,scwSource)
         ...
         if (scwDateValue.length==0)
            ...
            {function scwInputFormat()
                {var scwArrSeed = new Array(),
                     scwArrInput = scwDateValue.replace('>', '').replace('<', '').split(new RegExp('[\\'+scwArrDelimiters.join('\\')+']+','g'));
		...
		if((/msie/i.test(navigator.userAgent) && !/opera/i.test(navigator.userAgent)) && (document.location.host.toLowerCase().indexOf('campvega') >= 0)){
			margins = getBodyWidth() - 834;
			offsetTop -= 105;
			offsetLeft -= margins / 2 + 80;
		}
            ...
            {scwSelYears.options.selectedIndex=Math.floor(scwMonthSum/12);
             ...
             function scwSetOutput(scwOutputDate)
                {if (typeof scwTargetEle.value == 'undefined')
                     ...
                     var prefix = scwTargetEle.value.substring(0, 1);
                     if (prefix !== '>' && prefix !== '<') {
                         prefix = '';
                     }
                     scwTargetEle.value = prefix + scwOutputDate.scwFormat(scwDateOutputFormat);
                 }
                 scwHide();
                };
...
function scwSetLanguage(language){
	switch (language){
		case 'ru':
