...
class Kint
{
	 ...
	 * @return mixed
	 */
	public static function trace( $trace = null, $return = false )
	{
		if ( !Kint::enabled() ) return false;
 ...
 * @return string
 */
function kintLite( &$var, $level = 0, $maxLevel = 7 )
{
	...
	if ( $var === null ) {
		...
		if ( empty( $var ) ) {
			...
			foreach ( $var as $key => &$val ) {
				...
				$dump     = kintLite( $val, $level + 1, $maxLevel );
				$output[] = "{$key}{$dump}";
			}
		...
		if ( empty( $array ) ) {
			...
			foreach ( $array as $key => & $val ) {
				...
				$output[] = "$space$s$access $key -> " . kintLite( $val, $level + 1, $maxLevel );
			}
			unset( $objects[$hash] );
			$output[] = "$space}";
