...
class CancellationFee extends \TAccountChecker
{
	public $mailFiles = "resy/it-869426923.eml";
    public $reFrom = '@resy.com';
    public $reSubject = [
        'Cancellation fee for your reservation at',
    ];
    public $lang = 'en';

    public static $dictionary = [
        'en' => [
            'detectPhrase' => ['You have been charged a cancellation fee of'],
            'If you have any' => ['If you have any', 'If you have questions, please contact']
        ],
    ];
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($this->http->FindSingleNode("//text()[{$this->starts($this->t('detectPhrase'))}]")){
            $event->general()
                ->status('Cancelled')
                ->cancelled();
        }

        $eventName = $this->http->FindSingleNode("//text()[{$this->starts($this->t('detectPhrase'))}]", null, false, "/^{$this->opt($this->t('detectPhrase'))}[ ]+.+{$this->opt($this->t('by'))}(.+){$this->opt($this->t('for your reservation on'))}/u");
        ...
        $event->booked()
            ->start($this->normalizeDate($this->http->FindSingleNode("//text()[{$this->starts($this->t('detectPhrase'))}]", null, false, "/{$this->opt($this->t('for your reservation on'))}[ ]+(.+)\.$/u")))
            ->noEnd();
    ...
    {
        if ($this->http->XPath->query("//node()[{$this->contains(['is a member of Resy', 'est membre de Resy', 'é membro do Resy', 'est un membre de Resy'])}]")->length > 0
