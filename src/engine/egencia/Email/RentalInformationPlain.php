...
class RentalInformationPlain extends \TAccountChecker
{
    public $mailFiles = "";
...
//        $this->logger->debug($text);
        ...
        foreach (['Dep' => ['Pick-up'], 'Arr' => ['Drop-off']] as $prefix => $texts) {
            // Drop-off
            // March 20, 2023 12:00 pm
            // 5077 Patriot Highway, Fredericksburg, 22408, Virginia
            if (preg_match("/\n *{$texts[0]} *\n *(?<Date>.+\s+\d+:\d+(?:\s+[AP]M)?) *\n+\s*(?<Name>.+)/iu", $text, $m)) {
                if ($prefix == 'Dep') {
                    $r->pickup()
                        ->location($m['Name'])
...
//        $this->logger->debug('$str = '.print_r( $str,true));
        $in = [
            ...
            "/^\s*([[:alpha:]]+)\s+(\d+)\s*,\s*(\d{4})\s+(\d{1,2}:\d{2}(\s*[ap]m)?)\s*$/i",
        ];
        $out = [
            "$2 $1 $3, $4",
        ];
