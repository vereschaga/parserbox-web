...
class TAccountCheckerRedbox extends TAccountChecker
{
    ...
    public function LoadLoginForm(): bool
    {
        ...
        $this->http->GetURL('https://www.redbox.com/');
        $scriptURL = $this->http->FindPreg("/(\/static\/js\/main\.[^.]+\.js)/");
        ...
        $siteKey = $this->http->FindPreg('/RECAPTCHA_ENTERPRISE_SITE_KEY="([^"]+)/');

        if (!$siteKey) {
            return false;
        }

        $keyCaptcha = $this->parseCaptcha($siteKey);

        if ($keyCaptcha === false) {
            return false;
        }

        $apiKey = $this->http->FindPreg('/API_KEY="([^"]+)/');

        if (is_null($apiKey)) {
            return false;
        }
        $this->http->GetURL('https://www.redbox.com/rbweb/api/cart/getView?slim=1', ['x-redbox-apikey' => $apiKey]);
        ...
        $this->http->PostURL('https://www.redbox.com/rbweb/api/v2/account/login', json_encode($data), [
            'content-type'    => 'text/plain;charset=UTF-8',
            'x-redbox-apikey' => $apiKey,
        ]);
