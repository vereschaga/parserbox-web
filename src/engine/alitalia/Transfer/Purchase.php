...
class Purchase extends \TAccountCheckerAlitalia
{
    use ProxyList;
    ...
    public function LoadLoginForm()
    {
        ...
        if ($numberOfMiles <= 0 || $numberOfMiles > 70000 || $numberOfMiles % 1000 !== 0 || $numberOfMiles == 6000) {
            throw new \UserInputError("Number of purchased points should be lesser then 70,000 and divisible by 1000");
        }
    ...
    {
        $this->http->GetURL('https://www.alitalia.com/content/alitalia/alitalia-us/en/personal-area/dedicated-services.html');

        if (!$this->http->ParseForm('ssoGatewayBuy')) {
            return false;
        }
        $this->http->PostForm();
        $buyUrl = $this->http->currentUrl();

        // $cookies = $this->http->GetCookies('.alitalia.com', '/', true);
        // $this->http->Log('<pre>'.json_encode($cookies, JSON_PRETTY_PRINT).'</pre>', null, false);
        // $cookieswww = $this->http->GetCookies('www.alitalia.com', '/', true);
        // $this->http->Log('<pre>'.json_encode($cookieswww, JSON_PRETTY_PRINT).'</pre>', null, false);
        ...
        try {
            ...
            // $http2->http->removeCookies();
            // $http2->http->GetURL("https://www.alitalia.com/content/alitalia/alitalia-us/en/personal-area/dedicated-services.html");            $http2->http->GetURL("https://www.alitalia.com/en_us/personal-area/account-statement.html");

            // foreach ($cookies as $key => $value) {
            //     $http2->driver->manage()->addCookie(['name' => $key, 'value' => $value, 'domain' => ".alitalia.com"]);
            // }
            // foreach ($cookieswww as $key => $value) {
            //     $http2->driver->manage()->addCookie(['name' => $key, 'value' => $value, 'domain' => "www.alitalia.com"]);
            // }
            // $http2->http->GetURL('https://www.alitalia.com/content/alitalia/alitalia-us/en/personal-area/dedicated-services.html');
...
//            ];
//
//            $http2->http->PostURL($buyUrl, $buyData, $headers);

            // if (!($elem =$http2->waitForElement(\WebDriverBy::xpath('//div[@class="millemiglia__service__content" and contains(.,"BUY MILES")]')))){
            //     return $this->clear('can\'t find buy miles');
            // }
            // $elem->click();
            // if (!($elem = $http2->waitForElement(\WebDriverBy::xpath('//*[@id="ssoGatewayBuy"]/ancestor::div[1]')))){
            //     return $this->clear('can\'t find buy button');
            // }
            // $elem->click();

            // if ($elem = $http2->driver->switchTo()->alert())
            //     $elem->accept();

            // sleep(5);
            // $handles = $http2->driver->getWindowHandles();
            // $http2->driver->switchTo()->window($handles[0]);
            // $http2->driver->close();
            // $handles = $http2->driver->getWindowHandles();
            // $http2->driver->switchTo()->window($handles[0]);
            // sleep(5);
