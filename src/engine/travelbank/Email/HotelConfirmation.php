...
{
    public $mailFiles = "travelbank/it-742182273.eml, travelbank/it-751698925.eml, travelbank/it-753710533.eml";
    public $subjects = [
        'Reservation for',
    ];
    ...
    public static $dictionary = [
        'en' => [
            'Total' => ['Total (Due at Hotel):', 'Trip Total:'],
        ],
    ];
    ...
    public function HotelConfirmation(Email $email)
    {
        ...
        $hotelConfirmation = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'Confirmation Number:')]", null, true, "/^\D+\:\s*([A-Z\d]+)$/");
        ...
        if (preg_match("/^(?<currency>\D{1,3})\s*(?<price>[\d\.\,\']+)$/", $priceInfo, $m)) {
            ...
            if ($feesNodes !== null) {
                foreach ($feesNodes as $root) {
                    ...
                    $feeSum = $this->http->FindSingleNode("./descendant::td[2]", $root, true, '/^\D{1,3}([\d\.\,\']+)$/');
        ...
        $h->addRoom()
            ->setRate($this->http->FindSingleNode("//text()[contains(normalize-space(), 'Room Charge')]", null, false, "/\D*(\D{1,3}[\d\.\,\`]+\/\w*)\)/"))
            ->setType($this->http->FindSingleNode("//text()[normalize-space()='Room Type:']/ancestor::tr[1]/descendant::td[2]", null, false, "/^.+$/"));

        $travellers = $this->http->FindNodes("//text()[normalize-space()='Traveler:']/ancestor::tr[1]/descendant::td[2]", null, "/^([[:alpha:]][-.\/\'’[:alpha:] ]*[[:alpha:]])$/");
        ...
        $h->hotel()
            ...
            ->address($this->http->FindSingleNode("//text()[normalize-space()='Address:']/ancestor::tr[1]/descendant::td[2]", null, false));
        ...
        $cancellation = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'Know Before You Go')]/ancestor::table[1]/following-sibling::table[1]/descendant::tr[contains(normalize-space(), 'Cancelation Policy')]/following-sibling::tr[2]");
