...
class StarbucksExtensionVietnam extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    use TextTrait;
    ...
    public function getStartingUrl(AccountOptions $options): string
    {
        switch ($options->login2) {
            case 'Vietnam':
            default:
                return "https://card.starbucks.vn/en/profile";
        }
    }
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        if ($tab->evaluate('//iframe[@title="reCAPTCHA"]', EvaluateOptions::new()->timeout(5))) {
            ...
            $result = $tab->evaluate('
                //div//span[contains(@class,"form-error")]
                | //a[contains(text(),"SIGN OUT")]
            ', EvaluateOptions::new()->timeout(90));
        } else {
            $tab->evaluate('//button[@class="btn btn-primary"]')->click();
            $result = $tab->evaluate('
                //div//span[contains(@class,"form-error")]
                | //a[contains(text(),"SIGN OUT")]
            ');
