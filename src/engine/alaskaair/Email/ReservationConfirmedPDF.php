...
class ReservationConfirmedPDF extends \TAccountChecker
{
    public $mailFiles = "alaskaair/it-189760755.eml";
    public $lang = 'en';
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (strpos($text, 'Alaska ') !== false && strpos($text, 'E-Ticket:') !== false && strpos($text, 'Flight Total for') !== false
            ) {
                return true;
            }
        }
    ...
    public function ParseFlightPDF(Email $email, $text)
    {
        ...
        if (preg_match_all("/{$this->opt($this->t('Confirmation code:'))}\s*([A-Z]{6})\s+/i", $text, $m)) {
        ...
        $flightText = $this->re("/\n+(^\s*Flight\s*Departs\s*Arrives.+)Flight Total for/smu", $text);

        if (!empty($flightText)) {
            $flightParts = $this->split("/^(\s*\w+\s*\d{2,4})/mu", $flightText);

            if (preg_match_all("/{$this->opt($this->t('E-Ticket:'))}\s*(\d+)/", $text, $m)) {
            ...
            foreach ($flightParts as $flightPart) {
                if (stripos($flightPart, '(') !== false) {
                    ...
                    if (preg_match("/Flight\s*(?<airName>\D+)\s+(?<airNumber>\d{1,4})(\s*.+)?\s+(?<cabin>\w+)\s+\((?<bookingCode>[A-Z])\).+Distance:\s+(?<distance>.+)\s+\|\n+Duration\:\s*(?<duration>.+(?:h|m))(?:\n|$)/msu", $flightTable[0], $m)) {
                        $s->airline()
                            ->name($m['airName'])
                            ->number($m['airNumber']);
                    ...
                    if (preg_match("/Arrives\n.+\(\s*(?<arrCode>[A-Z]{3})\)\s*(?:[+]\s*)?(?<arrDate>\w+\,\s*\w+\s*\d+)\s*(?:(?<bookingCode>[A-Z])\s*\((?<cabin>\w+)\))?\s*(?<arrTime>[\d\:]+\s*a?p?m)/us", $flightTable[2], $m)) {
                        $s->arrival()
                            ->code($m['arrCode'])
                    ...
                    $seat = $this->re("/{$s->getDepCode()}\-{$s->getArrCode()}\s*(\d{1,2}[A-Z])/", $text);
            ...
            if (preg_match_all("/Traveler\n+(\D+)\n\s*E-Ticket/mu", $text, $m)) {
                $f->general()
                    ->travellers(array_unique($m[1]), true);
            }

            if (preg_match("/Flight Total for \d passenger\:\s*(?<currency>\D)(?<total>[\d\.\,]+)/u", $text, $m)) {
                $currency = $m['currency'];
                ...
                $cost = $this->re("/Base fare\s*\D([\d\.\,]+)/", $text);
                ...
                $tax = $this->re("/Taxes and fees\s*\D([\d\.\,]+)/", $text);
