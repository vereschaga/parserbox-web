...
class It1995087 extends \TAccountCheckerExtended
{
    public $rePlain = [
        ['#\n[>\s*]*From\s*:[^\n]*?tandt#i', 'us', ''],
    ];
    public $reHtml = "";
    public $rePDF = [
        ['#Travel[^\w]+and[^\w]+Transport#i', 'us', '/1'],
    ];
    public $reSubject = [
        ['#Travel and Transport#i', 'us', ''],
    ];
    public $reFrom = [
        ['#tandt#i', 'us', ''],
    ];
    ...
    public $upDate = "24.03.2015, 15:57";
    public $crDate = "";
    public $xPath = "";
    public $mailFiles = "transport/it-2134407.eml, transport/it-2514366.eml";
    public $re_catcher = "#.*?#";
        ...
        return [
            "#.*?#" => [
                "ItinerariesSplitter" => function ($text = '', $node = null, $it = null) {
                    $from = $this->parser->getFrom();
                    $from = is_string($from) ? $from : null;
                    $userEmail = strtolower(orval(
                        $from,
                        re("#\n\s*To\s*:[^\n]*?([^\s@\n]+@[\w\-.]+)#"),
                        $this->parser->getCleanTo()
                    ));
                ...
                "#^AIR#" => [
                    ...
                    "RecordLocator" => function ($text = '', $node = null, $it = null) {
                        return re("#\s+(?:Booking\s+Reference|Record\s+Locator)\s*:\s*([A-Z\d\-]+)#");
                    },
