...
var api = {
    ...
    _showFader: function (text) {
        ...
        message.innerHTML = "<tr><td><div style='background-image: url(//awardwallet.com/lib/images/progressCircle.gif); " +
...
var browserApiLogsQueue = [];
var logsTimeoutId = null;

function sendLogs(immediately) {
    if (logsTimeoutId) {
        clearTimeout(logsTimeoutId);
    }
    var cb = () => {
        if (browserApiLogsQueue.length > 0) {
            api.sendNativeEvent('logger', {
                message: browserApiLogsQueue.join('\n')
            });
            browserApiLogsQueue = [];
        }
    }
    if (immediately) {
        ...
        logsTimeoutId = setTimeout(cb, 500);
    }
}
...
var browserAPI = {
    log: function (message) {
        ...
        browserApiLogsQueue.push('[' + [date.getHours(), date.getMinutes(), date.getSeconds()].join(':') + '] ' + message)
        sendLogs()
    },
