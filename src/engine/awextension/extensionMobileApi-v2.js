...
var api = {
    ...
    error: function (text) {
        setTimeout(function () {
            document.location.href = "aw://error?" + encodeURIComponent(text).replace(/\'/g, "%27");
        }, 0)
    },

    log: function (text) {
        setTimeout(function () {
            document.location.href = "aw://log?" + encodeURIComponent(text).replace(/\'/g, "%27");
        }, 0)
    },
    ...
    complete: function () {
        setTimeout(function () {
            document.location.href = "aw://complete?params="+encodeURIComponent(JSON.stringify(params));
        }, 0)
    },

    getDepDate: function () {
        return new Date((typeof (params.depDate) != "undefined" && params.depDate) ? params.depDate * 1000 : Date.now());
    },

    getArrDate: function () {
        return new Date((typeof (params.arrDate) != "undefined" && params.arrDate) ? params.arrDate * 1000 : Date.now());
    },
    ...
    reCaptchaMessage: function (onlyOnce) {
        api.showFader('Message from AwardWallet: In order to log in into this account, you need to solve the CAPTCHA below and click the sign in button. Once logged in, sit back and relax, we will do the rest.', onlyOnce);
    },
    captchaMessageDesktop: function (onlyOnce) {
        api.showFader('Message from AwardWallet: To speed things up, please solve the CAPTCHA image below (if you see one) and click the sign in button. Once logged in, sit back and relax, we will do the rest.', onlyOnce);
    },
