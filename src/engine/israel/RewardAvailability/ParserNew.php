...
class ParserNew extends \TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        $this->http->saveScreenshots = false; // сильно увеличивает время парсинга, включать лучше точечно для дебага
    ...
    {
        try {
            $this->http->GetURL('https://matmid.elal.com/en/Login/Pages/Login.aspx');

            if ($this->waitForElement(\WebDriverBy::xpath("
               //h3[contains(text(), 'Access denied')] | //h1[contains(text(), 'Access Denied')]
           "), 3)) {
            ...
            $this->authData['memberNumber'] = $this->waitForElement(\WebDriverBy::xpath('(//label[@for="member_number"])[1]/../input'),
                5);
            $this->authData['password'] = $this->waitForElement(\WebDriverBy::xpath('(//label[@for="exampleInputEmail1"])[1]/../input'),
                0);
            $this->authData['submitBtn'] = $this->waitForElement(\WebDriverBy::xpath('(//input[@type="submit"])[1]'),
                0);

            if ($this->authData['memberNumber'] && $this->authData['password'] && $this->authData['submitBtn']) {
                $this->saveResponse();
    ...
    {
        try {
            $this->authData['memberNumber']->sendKeys($this->AccountFields['Login']);
            $this->authData['password']->sendKeys($this->AccountFields['Pass']);
            $this->authData['submitBtn']->click();
           ...
           "), 5)) {
            ...
            if ($this->waitForElement(\WebDriverBy::xpath('(//label[@for="member_number"])[1]/../input'), 5)) {
                throw new \CheckRetryNeededException(5, 0);
            }

            if ($this->waitForElement(\WebDriverBy::xpath("//div[contains(text(), 'Log off was not carried out.')]/a"),
                0)) {
                throw new \CheckException("Your login has been restricted.", ACCOUNT_LOCKOUT);
            }
        ...
        throw new \CheckRetryNeededException(5, 0);
        //переключение браузреа, если поиск перелетов в puppeteer не работает
        /*$cookies = $this->driver->manage()->getCookies();
        $this->logger->error('Cookies!: '.var_export($cookies, true));
        $this->http->cleanup();
        $this->UseSelenium();
        ...
        foreach ($cookies as $c) {
            $this->driver->manage()->addCookie(['name' => $c['name'], 'value' => $c['value'], 'domain' => $c['domain'], 'path' => $c['path']]);
        }
        $this->logger->error('NewCookies!: '.var_export($this->driver->manage()->getCookies(), true));*/
    }
...
//        $this->http->removeCookies();
    ...
    private function ParseReward(array $fields, ?bool $isRetry = false)
    {
        ...
        if ($this->isRouteAvailable === false) {
            return [];
        }

        /*$text = $this->waitForElement(\WebDriverBy::xpath(" //h3[contains(text(),'Departure')]"), 45);
        ...
        return $this->checkResult($fields, false, $this->id);
    }

    private function loadSessionRequest(array $fields)
    {
        $this->getSessionId();
        $this->checkRoute($fields);
...
//        $this->http->saveScreenshots = false;
        ...
        if (isset($dictionary[10]['bookingFlights']['fares']['benefits']) && is_array($dictionary[10]['bookingFlights']['fares']['benefits'])) {
            foreach ($dictionary[10]['bookingFlights']['fares']['benefits'] as $key => $benefit) {
                $mealCodes[$key] = $benefit;
            }
        }

        $this->logger->error('Meal codes: ' . var_export($mealCodes, true));
    ...
    private function checkResult(array $fields, ?bool $isRetry = false, string $id)
    {
        if (
            $this->http->FindSingleNode("//h1[contains(text(), 'This site can’t be reached')]")
            || $this->http->FindSingleNode("//span[contains(text(), 'This site can’t be reached')]")
        ...
        if (!$data || !isset($data['data'])) {
            for ($i = 0; $i < 15; $i++) {
                $this->getSessionId();
                $this->logger->notice('[wait ' . $i . ' seconds for seesionId]');

                if ((strpos($this->http->currentUrl(),
                            'https://booking.elal.com') !== false) && strlen($this->id) > 0) {
                    break;
                }
                sleep(1);
            }
            ...
            // old version - sometimes helps
            $this->logger->debug('sessionId: ' . $this->id);
            ...
            xhttp.open("GET", "https://booking.elal.com/ibe/rest/scopes/elal/modules/application,common,header,user,cart,formValidators,bookingPNR,errors,atc,bookingAncillaries,bookingFlights,bookingPassengers,search,manageMyBooking/translations/en", true);
            xhttp.setRequestHeader("Authorization", "Bearer ' . $this->id . '");
            xhttp.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
            xhttp.setRequestHeader("Accept", "application/json, text/plain, */*");
            xhttp.send();
            ');
            ...
            $this->logger->notice('run: ' . $script);
            $this->logger->error('current page: ' . $this->http->currentUrl());
            $this->driver->executeScript($script);
            ...
            foreach ($requests as $n => $xhr) {
                if (strpos($xhr->request->getUri(), '/search/points/outbound') !== false) {
...
//            if (isset($reAirportCheck) || isset($this->reloadStart)) { // на рестарте новой странице может врать.. пока дебаг
    ...
    private function loadStartPage($fields)
    {
        ...
        try {
            ...
            $this->waitForElement(\WebDriverBy::xpath("//button[@type='submit' and (contains(text(),'Find me a flight') or contains(@aria-label,'search'))]"),
                20);
    ...
    {
        $this->logger->notice(__METHOD__);
        $this->waitForElement(\WebDriverBy::xpath("//div[contains(@class,'ui-panel__wrapper') and not(contains(@class,'ui-panel__wrapper__hide'))]/div[contains(@class,'ui-panel__content')]/div"),
            45);
...
//        $this->logger->debug(var_export($codesDestination, true));
    ...
    private function loginSuccessful(bool $isBeforeLogin = false)
    {
        $this->logger->notice(__METHOD__);
        $time = 20;
        ...
        if ($this->waitForElement(\WebDriverBy::xpath("//button[contains(text(), 'Log Off')] | //div[@class='account-innerBox-content']/span[@class='name']"),
            $time)) {
            $this->logger->notice('login successful');

            if (strpos($this->http->currentUrl(), 'www.elal.com/eng/othercountries') === false) {
                $this->http->GetURL('https://www.elal.com/eng/othercountries');
            }
    ...
    private function checkRoute($fields)
    {
        $this->logger->warning('warning token: ' . $this->id);

        if (!empty($this->id) && !$this->validRoute($this->id, $fields)) {
