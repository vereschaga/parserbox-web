...
class TAccountCheckerAscott extends TAccountChecker
{
    ...
    public function LoadLoginForm(): bool
    {
        ...
        try {
            ...
            $selenium->http->GetURL('https://www.discoverasr.com/en');

            $this->catchLoginRequest($selenium);

            if ($cookieBtn = $selenium->waitForElement(WebDriverBy::xpath('//button[@id="truste-consent-button"]'), self::WAIT_TIMEOUT)) {
                $cookieBtn->click();
            }

            $signInButton = $selenium->waitForElement(WebDriverBy::xpath('//a[@class="signin-icon" and not(contains(@href, "javascript"))]'), self::WAIT_TIMEOUT);
            ...
            $submit = $selenium->waitForElement(WebDriverBy::xpath('//button[contains(text(), "Sign in") and not(contains(@class, "disabled"))]'), self::WAIT_TIMEOUT);
            $selenium->saveResponse();
            ...
            $selenium->waitForElement(WebDriverBy::xpath('//div[contains(@class, "form-alert-error")] | //div[contains(@class,"profile-dropdown")]'), self::WAIT_TIMEOUT);
    ...
    protected function parseCaptcha($selenium)
    {
        ...
        $captchaIframe = $selenium->waitForElement(WebDriverBy::xpath('//iframe[contains(@id, "captchaId")]'), 0);
        $selenium->driver->switchTo()->frame($captchaIframe);

        sleep(3); // the image doesn't load right away, there's no other way to catch it

        $selenium->saveResponse();

        $link = $selenium->http->FindSingleNode('//img[@class="mtcap-show-if-nocss"]/@src');
        $this->logger->debug('Captcha link: ' . $link);

        if (!isset($link)) {
            $this->logger->debug('Wrong link');
        ...
        $captcha = $this->recognizeCaptcha($this->recognizer, $link);

        if (!$captcha) {
            return false;
        }

        $captchaInput = $selenium->waitForElement(WebDriverBy::xpath('//input[contains(@id, "mtcap-inputtext")]'), 0);
        $captchaInput->sendKeys($captcha);

        if ($selenium->waitForElement(WebDriverBy::xpath('//div[contains(@class,"mtcap-msgbox")]/div/p[contains(text(), "Please try again..")]'), self::WAIT_TIMEOUT)) {
            $this->captchaReporting($this->recognizer, false);
