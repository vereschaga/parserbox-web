...
namespace AwardWallet\Engine\airfare\Email;

class Itinerary1 extends \TAccountChecker
{
    public $mailFiles = "airfare/it-1.eml, airfare/it-4981250.eml, airfare/it-5012826.eml, airfare/it-26675624.eml";
    public $processors = [];
    public $reText = null;
    public $reHtml = "#Live Itinerary#i";

    public $xInstance = null;
    public $lastRe = null;

    private $subjects = [
        'en' => ['Confirmation Email', 'Urgent regarding your flights', 'Regarding your flights'],
    ];
    ...
    public function __construct()
    {
        ...
        // Define processors
        $this->processors = [
            // Parsed file "airfare/it-1.eml"
            "#Live Itinerary#" => function (&$it, $parser) {
                ...
                // Passengers
                $passengers = $this->http->FindNodes("//text()[contains(.,'Adult')]/ancestor::td[1]/following-sibling::*/descendant::text()[normalize-space(.)]", null, '/^([A-z][-.\'A-z\s]*[A-z])$/');
                $passengers = array_values(array_filter($passengers));

                if (count($passengers)) {
                    $it['Passengers'] = array_unique($passengers);
                }
                //				$it['Passengers'] = $this->glue(trim(beautifulName($this->re("#\n\d+ Adult:\s+(.*?)\nLive Itinerary#ms", $text))));

                // TicketNumbers
                $ticketNumbers = $this->http->FindNodes("//text()[normalize-space(.)='Ticket(s) Number:']/ancestor::td[1]/following-sibling::*[normalize-space(.)][1]/descendant::text()[normalize-space(.)]", null, '/^([-\d]*\d{8,}[-\d]*)$/');
                ...
                //				$anchorDate = $this->mkDate($this->re("#\nDate:\s+([^\n]+)#ms", $text));
                $anchorDate = $this->mkDate($parser->getDate());
                $year = date('Y', $anchorDate);

                $xpathFragmentBold = '(self::b or self::strong)';

                $segments = $this->http->XPath->query("//h1/following-sibling::table//tr[not(contains(normalize-space(.),'Leg does not match the history'))]");

                foreach ($segments as $tr) {
                    $seg = [];

                    $seg['FlightNumber'] = $this->http->FindSingleNode("./td[5]", $tr);
                    $seg['Stops'] = $this->http->FindSingleNode("./td[6]", $tr);

                    $seg['AirlineName'] = $this->http->FindSingleNode("./td[2]/descendant::text()[normalize-space(.)][1]", $tr);
                    $seg['Operator'] = $this->http->FindSingleNode("./td[2]/descendant::text()[normalize-space(.)][2]", $tr, true, "/OPERATED BY (.*?)$/");

                    $seg['DepCode'] = $this->re("/^([A-Z]{3})-(.*?)$/", $this->http->FindSingleNode("./td[3]/descendant::*[{$xpathFragmentBold}][1]", $tr));
                    $seg['DepName'] = $this->re(2);
                    $seg['DepDate'] = $this->correctByDate($this->mkDate($this->re("#\w{3}\s+([^\s]+)\s(.*?)$#", $this->http->FindSingleNode("./td[3]/descendant::*[{$xpathFragmentBold}][1]/following::text()[normalize-space(.)][1]", $tr)) . $year . ', ' . $this->re(2)), $anchorDate);

                    $seg['ArrCode'] = $this->re("/^([A-Z]{3})-(.*?)$/", $this->http->FindSingleNode("./td[4]/descendant::*[{$xpathFragmentBold}][1]", $tr));
                    $seg['ArrName'] = $this->re(2);
                    $seg['ArrDate'] = $this->correctByDate($this->mkDate($this->re("#\w{3}\s+([^\s]+)\s(.*?)$#", $this->http->FindSingleNode("./td[4]/descendant::*[{$xpathFragmentBold}][1]/following::text()[normalize-space(.)][1]", $tr)) . $year . ', ' . $this->re(2)), $anchorDate);
    ...
    {
        return preg_match('/[.@]airfare\.com/i', $from) > 0;
    }

    public function detectEmailByHeaders(array $headers)
    {
        if (self::detectEmailFromProvider($headers['from']) !== true && stripos($headers['subject'], 'Airfare.com') === false) {
            return false;
        }
