...
class It2364340 extends \TAccountChecker
{
    public $mailFiles = "harrah/it-2364340.eml, harrah/it-2506876.eml, harrah/it-2509064.eml, harrah/it-2530042.eml, harrah/it-2532903.eml";
    ...
    {
        if ($this->http->XPath->query("//a[contains(@href,'.caesars-marketing.com')]")->length === 0) {
            return false;
        }
    ...
    private function parseHtml(Email $email)
    {
        ...
        // General
        $h->general()
            ->confirmation($this->http->FindSingleNode("//text()[" . $this->eq($this->t("Confirmation Number:")) . "]/following::text()[normalize-space()][1]", null, false, '/^\s*([A-Z\d]+)\s*$/'),
                "Confirmation Number")
            ->traveller($this->http->FindSingleNode("//td[" . $this->eq($this->t("Guest Name:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]"))
        ;
        ...
        // Hotel
        $name = $this->http->FindSingleNode("//text()[" . $this->eq($this->t("Hotel")) . "]/following::text()[normalize-space()][1]");

        if (!empty($name)) {
            $h->hotel()
                ->name($name)
            ;
            $address = implode("\n", $this->http->FindNodes("//text()[" . $this->eq($this->t("Hotel")) . "]/following::text()[" . $this->eq($name) . "]/following::text()[normalize-space()][position() < 7]"));

            if (preg_match("/(?<address>[\s\S]+?)\n\s*(?:1844IAMCROM)?(?<phone>[\d\+\(]{1,2}[\d\-\)\(\+ ]{4,})\n[\s\S]*" . $this->opt($this->t("Confirmation Number:")) . "/", $address, $m)) {
                $h->hotel()
        ...
        // Booked
        $h->booked()
            ->checkIn(strtotime($this->http->FindSingleNode("//td[" . $this->eq($this->t("Check-In:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]")))
            ->checkOut(strtotime($this->http->FindSingleNode("//td[" . $this->eq($this->t("Check-Out:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]")))
            ->rooms($this->http->FindSingleNode("//td[" . $this->eq($this->t("# of Rooms:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]"))
            ->guests($this->http->FindSingleNode("//td[" . $this->eq($this->t("Adults:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]"))
            ->kids($this->http->FindSingleNode("//td[" . $this->eq($this->t("Children:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]"))
        ;

        // Rooms
        $h->addRoom()
            ->setType($this->http->FindSingleNode("//td[" . $this->eq($this->t("Room Type:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]"))
        ;

        // Total
        $total = $this->http->FindSingleNode("//td[" . $this->eq($this->t("Grand Total:")) . "]/following-sibling::td[position() < 3][normalize-space()][1]");
